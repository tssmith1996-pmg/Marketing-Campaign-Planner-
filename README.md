...
Media plans now estimate costs for each line item using channel type, format, audience, and flight length so planners can compare projected totals alongside entered budgets.

Plans can also be exported as client-ready PDFs through the `Export` action, which generates a simple summary document on the fly.
